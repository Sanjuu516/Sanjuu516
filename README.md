...
### 🩸 [Blockchain Blood Donation System](https://github.com/Sanjuu516/blockchain-blood-donation)
> **Decentralized Blood Supply Chain & Provenance DApp**
- **Tech Stack:** Solidity `^0.8.0`, Web3.js, React 18, Node.js/Express, MongoDB Atlas, Nodemailer.
- **Key Engineering:**
  - Built an EVM Smart Contract (`BloodDonation.sol`) tracking 6 discrete lifecycle states (`Initiated` ➔ `Collected` ➔ `InTransit` ➔ `Arrived` ➔ `Processing` ➔ `Ready`), ensuring immutable blood unit provenance.
  - Developed a dual-portal React 18 interface (Donor Hub & Hospital Command Center) equipped with inventory analytics (Chart.js/Recharts).
  - Integrated an automated Nodemailer broadcast engine that sends emergency alerts to hospital networks upon request creation.
  - Implemented an intelligent AI consultation chatbot (**BloChatbot**) for donor health eligibility lookups.
...
- **Key Engineering:**
  - Designed a transparent verification ledger for tracking organic and herbal products from farm to end consumer.
  - Enforced verifiable quality checkpoints on-chain to prevent counterfeiting and verify organic certifications.
